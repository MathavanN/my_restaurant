...
namespace MyRestaurant.Business.Repositories
{
    public class PurchaseOrderItemRepository : IPurchaseOrderItemRepository {
        ...
        private async Task CheckOrderItemAsync(long id, long itemId)
        {
            var dbStockType = await _purchaseOrderItem.GetPurchaseOrderItemAsync(d => d.ItemId == itemId && d.Id != id);
            if (dbStockType != null)
        ...
        {
            await CheckOrderItemAsync(0, purchaseOrderItemDto.ItemId);
